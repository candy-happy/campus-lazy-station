...
router.post('/report', requireAuth, (req, res) => JSON_RES(res, () => {
  ...
  if (!['item', 'comment'].includes(target_type)) return makeError('举报类型无效');
  // 防止重复
  const existing = db.prepare('SELECT id FROM reports WHERE source=? AND target_type=? AND target_id=? AND reporter_phone=?').get('market', target_type, target_id, phone);
  if (existing) return makeError('您已举报过该内容');
  db.prepare(`INSERT INTO reports (source,target_type,target_id,target_content,reporter_phone,reason,detail,status,created_at) VALUES ('market',?,?,?,?,?,?,'pending',datetime('now','localtime'))`)
