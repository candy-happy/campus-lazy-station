...
const API = {
  ...
  async wallPostDetail(id) { const phone = localStorage.getItem('lazy_phone') || ''; return fetch('/api/wall/posts/' + id + (phone ? '?phone=' + encodeURIComponent(phone) : ''), { headers: this._headers() }).then(r => r.json()); },
