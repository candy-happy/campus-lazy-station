...
const API = {
  ...
  async campusStarShare(id) { return fetch('/api/campus-star/share/' + id, { method: 'POST', headers: this._headers() }).then(r => r.json()); },
  async campusStarReport(id, reason) { return fetch('/api/campus-star/report/' + id, { method: 'POST', headers: this._headers(), body: JSON.stringify({ reason }) }).then(r => r.json()); },
